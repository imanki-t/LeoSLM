...
ROOT CAUSE OF CRASH:
  xmp.spawn() uses ProcessPoolExecutor which registers workers SEQUENTIALLY.
  PJRT on v5e-8 requires ALL 8 to register SIMULTANEOUSLY → "Expected 8, got 1"

FIX: torch_xla.launch() — the modern PJRT-native API. Auto-detects world size
  and does simultaneous worker registration correctly for all TPU versions.
  Official migration: xmp.spawn(_fn) → torch_xla.launch(_fn)
"""
...
def load_ckpt(model, path):
    ...
    raw.load_state_dict(st, strict=False)
...
def run_phase(phase, rank, model, optimizer, dataset, loss_fn,
    ...
    if phase == 5:
        ...
        agt = AgenticGRPO(cfg=raw.cfg, model=model, optimizer=optimizer)

    total = max(1, len(dataset) // (bs * max(world_size,1))) * epochs
    ...
    for epoch in range(epochs):
        ...
        for batch in loader:
            ...
            set_lr(optimizer, cosine_lr(step, pc["warmup"], total, lr_hi, lr_lo))

            if phase == 5 and dpo:
                out = model(ids)
                U   = out["uncertainty"].mean(dim=1)
                lo, hi = (0,1) if U[0] <= U[1] else (1,0)
                co = {k:v[lo:lo+1] for k,v in out.items() if isinstance(v,torch.Tensor) and v.shape[0]==2}
                ro = {k:v[hi:hi+1] for k,v in out.items() if isinstance(v,torch.Tensor) and v.shape[0]==2}
                loss, metrics = dpo(co, ro, ids[lo:lo+1], ids[hi:hi+1])
            elif phase == 6 and grpo:
                metrics = grpo.grpo_step(batch, device); loss = metrics["total"]
            elif phase == 8 and agt:
                metrics = agt.agentic_grpo_step(batch, device); loss = metrics["total"]
            else:
                out, (loss, metrics) = model(ids), loss_fn(model(ids), ids, model=raw, phase=phase)
                out = model(ids); loss, metrics = loss_fn(out, ids, model=raw, phase=phase)
            ...
            if step % 10 == 0:
                m = {k:(v.item() if isinstance(v,torch.Tensor) else v) for k,v in metrics.items()}
            ...
            if smoke and step >= start_step + 50:
                xm.master_print("  Smoke: 50 steps ✓"); return step
...
def _mp_fn(rank: int, flags: dict):
    ...
    if rank == 0:
        ...
        xm.master_print(f"  {pc['total']/1e9:.2f}B params | device={device} | world={xm.xrt_world_size()}")
    ...
    if not Path(flags["train_data"]).exists():
        xm.master_print(f"  ERROR: {flags['train_data']} not found — run Cell 3")
        return

    dataset = LeoDataset(flags["train_data"], max_seq_len=4096, pad_id=cfg.pad_id)
    if rank == 0:
        xm.master_print(f"  Dataset: {len(dataset.data):,} tokens | vocab={cfg.vocab_size:,}")
    ...
    for phase in phases:
        if flags["resume"] and phase < start_phase:
            xm.master_print(f"  Skip phase {phase}"); continue
        step = run_phase(phase=phase, rank=rank, model=model, optimizer=optimizer,
...
def main():
    ...
    print(f"  Phase: {args.phase or 'ALL'} | GradCkpt: {'ON' if args.grad_ckpt else 'OFF'}")
    print(f"{'='*60}\n")

    if XLA_AVAILABLE:
        # FIX: torch_xla.launch() replaces xmp.spawn()
        # Handles simultaneous PJRT worker registration — works on v5e-8
