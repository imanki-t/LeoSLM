...
Usage:
    python3 prep_data_v4.py --tok-only   # Train tokenizer only (fast, ~10 min)
    python3 prep_data_v4.py --fast       # Tokenizer + 200M tokens (~25 min)
    python3 prep_data_v4.py --medium     # Tokenizer + 500M tokens (~1 hr)
    python3 prep_data_v4.py              # Full 2.1B tokens (~5 hrs)
"""
...
SPECIAL_TOKENS = [
    ...
    "[BUDGET]",     # 65537 — think budget forcing token
]

VOCAB_SIZE = 65536 + len(SPECIAL_TOKENS)  # = 65545 → but we target 65536 base + 9 = 65545
# Simpler: use 65536 total including specials → base BPE = 65527
BASE_VOCAB  = 65536 - len(SPECIAL_TOKENS)  # 65527 BPE merges
...
if args.tok_only:
    print("\n🦁 Tokenizer training complete! Run full prep with: python3 prep_data_v4.py")
    sys.exit(0)
...
try:
    ...
    print(f"   ⚠️  {e} — python-edu fallback...")
...
np.save("data/train_v4.npy", train_arr)
np.save("data/val_v4.npy",   val_arr)

train_gb = os.path.getsize("data/train_v4.npy") / 1e9
val_mb   = os.path.getsize("data/val_v4.npy")   / 1e6
...
print(f"\n🦁 V4 data ready! Run: python3 train_v4.py")
